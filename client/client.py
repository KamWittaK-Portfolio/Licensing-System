...
def login():
    ...
    if user == EXPECTED_USER and user_hash == EXPECTED_HASH:
        token = jwt.encode({"user": user, "exp": datetime.datetime.utcnow() + datetime.timedelta(minutes=30)}, JWT_KEY)
...
@app.route("/examples/cs")
@token_required
def examples_cs():
    return render_template("examples_cs.html")

@app.route("/examples/cpp")
@token_required
def examples_cpp():
    return render_template("examples_cpp.html")

@app.route("/examples/py")
@token_required
def examples_py():
    return render_template("examples_py.html")

@app.route("/examples/js")
@token_required
def examples_js():
    return render_template("examples_js.html")

@app.route("/examples/lua")
@token_required
def examples_lua():
    return render_template("examples_lua.html")
